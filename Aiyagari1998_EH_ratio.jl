...
#=
 ...
 =#
#=
Goal: you need to understand how different parameters shift when you adjust gdp/debt ratio 
b= gdp/debt ratio

equation 5 in main paper
τ= γ + (r-g)b 
...
#------------------------------------------------------------------------------------------------
    ...
    """
    function setupgrids_shocks!(HH::HHModel,b, curv=1.7)
        @unpack a̲,a̅,Na,ρ_ϵ,σ_ϵ,Nϵ,k,r̄,w̄,β,η,g, γ    = HH
        ...
        for s in 1:Nϵ
            ...
            #c = @. r̄*a + w̄*HH.ϵ[s] + (-τ)
            # l = @. (η/(1-η)) * w̄ * HH.ϵ[s] 
            c= @. max(1/(η)*( (r̄-g)*a + w̄*HH.ϵ[s] + (-τ)  ),0) # I interpreted these as the budget contraient where a = a' and we plug in static FOC
            l = @.max(((1-η)*c)/(w̄*HH.ϵ[s]* η     ),0) # static FOC
            V = U(HH,c,l)./(1-β)
    ...
    """
    function setupgrids_shocks!(AM::AiyagariModel,b,curv=2.)
        @unpack HH,Ia,N̄= AM
        @unpack a̲,a̅,Nϵ = HH
        setupgrids_shocks!(HH,b)
...
#------------------------------------------------------------------------------------------------
    ...
    """
    function iterate_endogenousgrid(HH,a′grid,cf′,b)
        @unpack γ,ϵ,β,Nϵ,Π,r̄,w̄,a̲,g,η = HH
        ...
        a = ((c .+ w̄.*ϵ'.*l .+ ((1+g) * a′grid)) .- w̄ .* ϵ' .- (-τ)) ./ (1+r̄) 
    ...
    """
    function solveHHproblem_eg!(HH,b)
        a′grid = nodes(HH.cf[1].basis)[1]#Get nodes for interpolation

        cf′ = iterate_endogenousgrid(HH,a′grid,HH.cf,b)
        diff = 1.
        while diff  > 1e-8
            HH.cf = iterate_endogenousgrid(HH,a′grid,cf′,b)
...
# finding how debt/GDP ratio effects aggregate hours, interest rates, welfare
...
#------------------------------------------------------------------------------------------------
    ...
    function calibratesteadystate!(AM,b)
        ...
        setupgrids_shocks!(AM,b)
        function βres(β)
            AM.HH.β=β
            # solvebellman!(AM.HH)      # collocation
            solveHHproblem_eg!(AM.HH,b)   # Endogenopus grid
...
#------------------------------------------------------------------------------------------------    
    function capital_supply_demand(AM,R,b)
        @unpack Θ̄,α,N̄,δ = AM
        AM.R̄ = R
        AM.HH.r̄ = R-1
...
#------------------------------------------------------------------------------------------------  
    AM = AiyagariModel()
    b= .6
    setupgrids_shocks!(AM,b) # this works!


    # this all runs but looks terrible
    # Figure 1: capital steady state: 
    Rgrid = LinRange(1.,1.007,10)
    KSKD = hcat([capital_supply_demand(AM,R,b) for R in Rgrid]...)
